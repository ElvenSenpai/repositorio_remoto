...
//Contar el total de animales que hay en el zoo

const countAnimals = () => {
        zoo.species.reduce((acc, animalType) => {
        return acc + animalType.residents.length
    }, 0)
}

//buscador empleado por ID

const searchID = (id) => {
    return zoo.employees.find(employee => employee.id === id)
}

//buscar empleados por IDs

const empleados = ['c5b83cb3-a451-49e2-ac45-ff3f54fbe7e1', '56d43ba3-a5a7-40f6-8dd7-cbb05082383f', 'b0dc644a-5335-489b-8a2c-4e086c7819a2']

const buscarEmpleados = (ids) => {
...
//funcion que saque un array de nombres de todos lo animales

const getNames = () => {
    const nombres = []
    species.forEach(anymalType => {
        const typeNames = anymalType.residents.map(resident => resident.name)
...
const getTicketTotal = (info, zoo) => {
    //reduce
    return Object.entries(info).reduce((acc, entry) => {
